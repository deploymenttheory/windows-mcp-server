...
| `packages` | | `Package` (list / search / install / uninstall via winget and MSI) — downloads run outside the egress proxy; in no persona |
...
| **Local user** — non-admin, same session | Read what the agent reads; use the running server | Credentials never returned to the model; the `FileSystem` tool refuses the credentials file, the audit key, the audit log, the policy document and the kill-switch control directory; `GuardrailStatus`/`Kill` cannot be removed | A user already holds their own privileges; the server does not raise them. **The protected-path check binds the `FileSystem` tool only** — `PowerShell` reaches the same files unchecked, which is why the `shell` toolset requires an explicit acknowledgement to be served alongside credentials. `Registry` and `ScheduledTask` now live in the non-default `system-admin` toolset |
