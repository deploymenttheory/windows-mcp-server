...
func TestKeyedChainCarriesValidMACs(t *testing.T) {
	...
	sink := &memSink{}
	log := NewAuditLog(sink, WithHMACKey(key))
	for i := 0; i < 4; i++ {
	...
	if err := VerifyMAC(sink.entries, key); err != nil {
	...
	if err := VerifyMAC(sink.entries, []byte("the-wrong-key")); err == nil {
...
func TestUnkeyedChainOmitsMAC(t *testing.T) {
	...
	if sink.entries[0].Mac != "" {
		t.Error("an unkeyed entry must have no MAC")
	}
	b, _ := json.Marshal(sink.entries[0])
	...
	if err := VerifyMAC(sink.entries, []byte("k")); err == nil {
...
func TestEmptyKeyLeavesLogUnkeyed(t *testing.T) {
	...
	log := NewAuditLog(sink, WithHMACKey([]byte("")))
	log.Append("e", nil)
	if sink.entries[0].Mac != "" {
