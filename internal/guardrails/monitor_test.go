...
func driftDecision() Decision {
	return Decision{Admit: false, Reasons: []string{"secure-boot=fail"}}
}
...
func TestMonitorReportOnlyTripKeepsMonitoring(t *testing.T) {
	...
	StartMonitor(ctx, MonitorConfig{
		...
		Evaluate: func(context.Context) Decision {
			evals.Add(1)
...
func TestMonitorExitsWhenStopped(t *testing.T) {
	...
	StartMonitor(ctx, MonitorConfig{
		Interval: 10 * time.Millisecond,
		Evaluate: func(context.Context) Decision { return driftDecision() },
