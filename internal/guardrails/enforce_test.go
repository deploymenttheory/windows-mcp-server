...
func newEnforceHarness(t *testing.T, policyJSON string, outcomes map[string]Status) *enforceHarness {
	t.Helper()
	e, _ := newTestEngine(t, policyJSON, outcomes)
	h := &enforceHarness{engine: e, sink: &memSink{}}
	next := func(context.Context, string, mcp.Request) (mcp.Result, error) {
		h.reached.Add(1)
		return &mcp.CallToolResult{Content: []mcp.Content{&mcp.TextContent{Text: "ok"}}}, nil
	}
	h.handler = e.Middleware(EnforcerDeps{
		Audit: NewAuditLog(h.sink),
...
func TestDenyOnToolCallIsAnIsErrorResult(t *testing.T) {
	h := newEnforceHarness(t, denyPolicy, map[string]Status{"bitlocker": Fail})
...
func TestKillVerdictTripsOnceAndStillRefuses(t *testing.T) {
	...
	h := newEnforceHarness(t, killPolicy, map[string]Status{"bitlocker": Fail})
...
func TestWarnProceedsAndTellsTheCaller(t *testing.T) {
	...
	h := newEnforceHarness(t, warnPolicy, map[string]Status{"bitlocker": Fail})
...
func TestAuditRecordsEveryDecisionIncludingAllows(t *testing.T) {
	h := newEnforceHarness(t, denyPolicy, map[string]Status{"bitlocker": Pass})
	...
	if err := VerifyChain(h.sink.entries); err != nil {
...
func TestAuditModeRecordsWhatEnforcingWouldHaveDone(t *testing.T) {
	...
	h := newEnforceHarness(t, auditPolicy, map[string]Status{"bitlocker": Fail})
