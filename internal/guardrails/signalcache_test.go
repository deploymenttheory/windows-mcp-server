...
func newCountingRegistry(ids ...string) *countingRegistry {
	...
	for _, id := range ids {
		...
		c.status[id] = Pass
		c.reg.Register(Guardrail{ID: id, Check: c.check(id)})
	}
	return c
}

func (c *countingRegistry) check(id string) CheckFunc {
	return func(context.Context, *Env) Result {
		c.counts[id].Add(1)
		c.mu.Lock()
		defer c.mu.Unlock()
		return Result{ID: id, Status: c.status[id]}
	}
}

func (c *countingRegistry) set(id string, s Status) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.status[id] = s
}
...
func testCache(t *testing.T, policyJSON string, ids ...string) (*signalCache, *countingRegistry, *time.Time) {
	...
	c := newSignalCache(cr.reg, p, func() *Env { return &Env{Sys: nil} })
...
func TestCacheReusesAReadingInsideItsTTL(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		if got := c.Read(ctx, "bitlocker", nil); got.Status != Pass {
...
func TestCacheStartsUnreadNotPassing(t *testing.T) {
	...
	for _, r := range c.Snapshot() {
		if r.Status == Pass {
			t.Errorf("signal %q reports Pass before it was ever evaluated", r.ID)
		}
	}
...
func TestCachePicksUpDrift(t *testing.T) {
	...
	*clock = clock.Add(30 * time.Second)
	if got := c.Read(ctx, "bitlocker", nil); got.Status != Fail {
...
func TestReadOfAnUndeclaredSignalSkips(t *testing.T) {
	...
	if got.Status != Skip {
		t.Errorf("undeclared signal status = %v, want %v", got.Status, Skip)
	}
}
