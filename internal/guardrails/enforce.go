...
func (e *Engine) Middleware(deps EnforcerDeps) mcp.Middleware {
	logger := deps.Logger
	if logger == nil {
		logger = slog.Default()
	}
	return func(next mcp.MethodHandler) mcp.MethodHandler {
		return func(ctx context.Context, method string, req mcp.Request) (mcp.Result, error) {
			subj, decidable := e.subjectFor(method, req)
			if !decidable {
				return next(ctx, method, req)
			}

			v := e.Evaluate(ctx, subj)
			e.record(deps, logger, v)

			switch {
			case v.Severity >= SeverityKill:
				// Audited above, before any containment.
				if deps.Kill != nil {
					deps.Kill("policy: " + v.Subject + ": " + v.Reason())
				}
				return e.refuse(method, v)
			case v.Severity >= SeverityDeny:
				return e.refuse(method, v)
			case v.Severity == SeverityWarn:
...
func (e *Engine) record(deps EnforcerDeps, logger *slog.Logger, v Verdict) {
...
func (e *Engine) refuse(method string, v Verdict) (mcp.Result, error) {
...
func attachWarning(res mcp.Result, v Verdict) mcp.Result {
...
func (e *Engine) subjectFor(method string, req mcp.Request) (Subject, bool) {
	switch method {
	case methodCallTool:
		...
		return e.SubjectForTool(method, p.Name), true
...
func dataEgressSubject(method, name string) Subject {
	return Subject{
		Scope:  ScopeCall,
		Method: method,
		Facts:  ToolFacts{Name: name, ReadOnly: true},
	}
}
